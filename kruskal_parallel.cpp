...
void merge(vector<tuple<int, int, int>>& arr,int l,int mid ,int r){
    int pos = 0;
    int i =l;int j = mid+1;
	vector<tuple<int, int, int>> c(r-l+1);
    while(i<=mid&&j<=r){
...
void mergeSort(vector<tuple<int, int, int>>& arr,int l,int r){
    if(l == r){
        return;
    }
    int mid = l + (r-l)/2;
